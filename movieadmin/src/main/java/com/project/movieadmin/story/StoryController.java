...
//  이미지 파일
		if (vo.getFile() == null || vo.getFile().isEmpty()) {
			...
			/*
			 * 오디오 파일 확장자 목록 (나중에 추가해보자 원래는 비디오에 mp3 파일을 넣으면 안 좋다 함) List<String>
			 * audioExtensions = new ArrayList<>(Arrays.asList("mp3", "wav", "flac", "ogg",
			 * "aac")); audioExtensions.add("dsd");
			 */
			...
			if (imageExtensions.contains(fileExtension)) {
				// 이미지 파일 처리
				String save_name = "img_" + System.currentTimeMillis()
...
//			     // 썸네일 생성 코드 추가
	...
	public String s_updateOK(StoryVO vo) throws IllegalStateException, IOException {
		...
		if (vo.getFile() == null || vo.getFile().isEmpty()) {
			log.error("이미지 파일과 동영상 파일이 모두 없습니다.");
			return "redirect:s_update.do"; // 글 작성 페이지로 리다이렉트
		}
//  이미지 파일
		if (vo.getFile() == null || vo.getFile().isEmpty()) {
			log.info("이미지 파일이 없거나 비어 있습니다.");
...
//			     // 썸네일 생성 코드 추가
		...
		if (result == 1) {
			return "redirect:s_selectOne.do?story_num="+vo.getStory_num()+"&nickname="+vo.getNickname();
		} else {
			return "redirect:s_update.do?story_num="+vo.getStory_num()+"&nickname="+vo.getNickname();
		}
