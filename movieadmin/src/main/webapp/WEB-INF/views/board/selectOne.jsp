			...
			<tbody>
				...
				<tr>
					<td colspan="5"><input type="hidden" name="board_num"
						...
						id="nickname"> <input type="button" value="추천 ${vo2.good}"
						class="b_increaseGood"> <input type="button"
						id="reportBtn" class="report"
						onClick="showDialogReport('${vo2.board_num}','${nickname}')"
						value="신고" /></td>
					<c:if test="${nickname == vo2.nickname}">
						<td align="right"><form
								action="b_update.do?board_num=${param.board_num}&nickname=${param.nickname}">
								<input type="hidden" name="board_num" value="${vo2.board_num}"><input type="submit" value="글 수정">
							</form> <form action="b_delete.do?board_num=${param.board_num}"><input type="submit" value="글 삭제"></form></td>
					</c:if>
				</tr>
